...
P = -60;                         %pressure drop of the entire tube length
L = 10^(-2);                      %tube length
...
RBCThixotropicMigration = @(t,AA,m,dpdz,B, B1, B2, Bz, B1z, B2z, B1zor, Br, B1r, B2r,B1ror, Bor, B1or, B2or,...
    P, P1, P2, Por, P1or, P2or,r,params) ...
        fRBCThixotropicMigration(t,AA,m,dpdz,B,B1, B2, Bz, B1z, B2z, B1zor, Br, B1r, B2r,B1ror, Bor, B1or, B2or,...
    P, P1, P2, Por, P1or, P2or,r,params);   
...
opts = odeset('RelTol',eps,'AbsTol',1/(2^m+1));
vz_fully_developed = -((1-r.^2)/4*dpdz(0));
newtonian_init = 0*vz_fully_developed/Bz;

RBCmigration_init = [newtonian_init, ... %omega*R0*newtonian(end,:),...
                    [0 zeros(1,m-1)],...
                    [params.phi_hct0 zeros(1,m-1)],...
                    [0 zeros(1,m-1)]];

RBCMigration = @(t, AA) RBCThixotropicMigration(t,AA,m,dpdz,B,B1, B2, Bz, B1z, B2z, B1zor, Br, B1r, B2r,B1ror, Bor, B1or, B2or,...
    P, P1, P2, Por, P1or, P2or,r,params);
...
vz_stressmigration = omega*R0*(filter.*RBCmigration(:,1:m))*Bz;
gamma_stressmigration = omega*(filter.*RBCmigration(:,1:m))*B1z;
C_stressmigration = (filter.*RBCmigration(:,m+1:2*m))*Br;
phi_stressmigration = (filter.*RBCmigration(:,2*m+1:3*m)*P);
M_stressmigration = (filter.*RBCmigration(:,3*m+1:4*m)*B);
phi = phi_stressmigration;
vz = vz_stressmigration;
lambda = (phi - M_stressmigration)./phi;